...
header_fields = ['name','isservice','categ_id','kyat','thb','usd']
...
#                          \n\nPlease check your " + message + " !"))                        
    ...
    def import_data(self):
        ...
        htl_service_cat_obj = self.env['hotel.service.type']
        # htll_res_partner = self.env['res.partner']
        htl_payment_type = self.env['payment.type']
        htl_services_obj = self.env['hotel.services']
...
#             uo_id = '1'
        ...
        if self.err_log <> '':
            ...
            for data in all_data:
                ...
                if categ_id:
                    print "hi"
                    categ_ids = htl_service_cat_obj.search([('name','=',categ_id)])
                    if not categ_ids:
                        categ = htl_service_cat_obj.create({'name': categ_id}).id    
                    else:
                        htl_service_cat_obj.write({'name': categ_id})
                        categ = categ_ids.id
                ...
                if product_tmp_id:
                    product_ids = htl_product_product.search([('product_tmpl_id','=',product_tmp_id),('isservice','=',1)])
                    if not product_ids:
                ...
                if product_id and  categ_id:
                    print "lohi"
                    service_ids = htl_services_obj.search([('product_id','=',product_id),('categ_id','=',categ)])
                    print "name is exit , ", service_ids
                    if not service_ids:
                        serv_val = {
                            'product_id': product_id,
                            'categ_id':categ
                        }
                        services = htl_services_obj.create(serv_val)
                        create_count += 1
                    else:
                        print "name is exit , ",service_ids
                        serv_val = {
                            'product_id': product_id,
                            'categ_id':categ}
                        htl_services_obj.write(serv_val)
                        services = service_ids
                        print "Product is Updated...."
                        update_count += 1
                if services.id :
                    prod = htl_services_obj.search([('product_id','=',product_id)])
                    if prod:
                        val={'isservice':True}
                        htl_product_product.write(val)

                if services and kyat_price or usd_price or thb_price:
                    if kyat_price:
                        currency_id = htl_currency_obj.search([('name','=','MMK')])    
                        saleprice_ids = htl_payment_type.search([('service_charges','=',services.id),
                            ('currency','=',currency_id.id),('price','=',kyat_price)]) 
                        if not saleprice_ids:
                            vals={'service_charges':services.id,
