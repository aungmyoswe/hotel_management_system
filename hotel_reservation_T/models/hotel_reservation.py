...
#                         reservation_line_obj.create(vals)
    ...
    def create_folio(self):
        ...
        for reservation in self:
            ...
            checkin_date = temp_date
            checkout_date = temp_date + tm_delta 
            checkin_time = datetime.strftime(checkin_date,'%Y-%m-%d %H:%M:%S')
            in_check = datetime.strftime(checkin_date,'%Y-%m-%d 05:30:00')
            if checkin_time < in_check:
                checkin_date =checkin_time
                checkout_date = in_check
            else:
