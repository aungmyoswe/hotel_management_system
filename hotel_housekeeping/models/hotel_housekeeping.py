...
class HotelHousekeeping(models.Model):
    ...
    def room_clean(self):
        ...
        print "room_id",self.room_no.id
        room_ids = room_obj.search([('id','=',self.room_no.id),('status','=','occupied')])
        print room_ids,"room"
        if room_ids:
            product_id = product_obj.search([('id','=',room_ids.product_id.id)])
