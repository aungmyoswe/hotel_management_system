...
class ProductProduct(models.Model):
    ...
    isroom = fields.Boolean('Is Room',store=True)
    iscategid = fields.Boolean('Is categ id',store=True)
    isservice = fields.Boolean('Is Service id',store=True)
...
class HotelRoom(models.Model):
    ...
    product_manager = fields.Many2one('res.users', string='Product Manager')
    charges_line = fields.One2many('payment.type',"room_charges","Room charges",store=True,required=True)
    # folios = fields.Many2many("hotel.folio","folio","folio_id","Room")
...
class HotelFolio(models.Model):
    ...
    def _get_checkin_date(self):
        ...
        return _offset_format_timestamp1(time.strftime("%Y-%m-%d %H:%M:%S"),
    ...
    def _get_room(self):
        ...
        room = room_obj.search([('status','!=','occupied')])
        if room:
            avail_prod_ids.append(room)
    ...
    uncheck = fields.Boolean("Uncheck",default=False)
    agency = fields.Many2one("res.partner",domain="[('agency','=',True)]",string="Agency",store=True)
    currency = fields.Many2one("res.currency",required=True,default=_get_currency,string="Currency")
    currency_amount = fields.Float(string="Currency Rate",store=True)
    type_payment = fields.Many2one("pay.category",string="Payment Type",store=True)
    room_ids = fields.Many2one("hotel.room",string="Room No",default=_get_room,store=True,readonly=False)
    

    @api.multi
    def _invoices(self):
        sale_obj = self.env['sale.advance.payment.inv']
        res = sale_obj.create({'name':"Name"})
        res.create_invoices_2()
        return res
    ...
    @api.multi
    def _invoices_2(self):
        sale_obj = self.env['sale.advance.payment.inv']
        res = sale_obj.create({'name':"Name"})
        res.create_invoices_3()
        return res

    #add onchange room by aungmyoswe 20181114
    @api.model
    @api.onchange('room_ids')
    def onchange_room_ids(self):
        hotel_room_obj = self.env['hotel.room']
        hotel_paytype_obj = self.env['payment.type']
        parnter_line_obj = self.env['res.partner.line']
        if self.room_ids:
            hotel_room_ids = hotel_room_obj.search([('id','=',self.room_ids.id)])
            product_id= hotel_room_ids.product_id
            if self.agency:
                print self.agency,hotel_room_ids.categ_id.id,product_id.categ_id.id,"type"
                partner_line_id = parnter_line_obj.search([('partner_line_id','=',self.agency.id),('room_type','=',hotel_room_ids.categ_id.id),('currency','=',self.currency.id)])
                print partner_line_id.price,"Price"
                if self.room_lines:
                    for room in self.room_lines:
                        room.product_id = product_id
                        room.name= product_id.name
                        room.price_unit = partner_line_id.price
                        room.product_uom = product_id.uom_id
                        room.currency = self.currency
                        tax_obj = self.env['account.tax']
                        pr = product_id
                        room.price_unit = tax_obj._fix_tax_included_price(partner_line_id.price,
                                                                      pr.taxes_id,
                                                                      room.tax_id)
            else:
                pay_ids = hotel_paytype_obj.search([('currency','=',self.currency.id),('room_charges','=',self.room_ids.id)])
                if self.room_lines:
                    for room in self.room_lines:
                        room.product_id = product_id
                        room.name= product_id.name
                        room.price_unit = pay_ids.price
                        room.product_uom = product_id.uom_id
                        room.currency = self.currency
                        tax_obj = self.env['account.tax']
                        pr = product_id
                        room.price_unit = tax_obj._fix_tax_included_price(pay_ids.price,
                                                                      pr.taxes_id,
                                                                      room.tax_id)

    # add onchange agency by aungmyoswe 20181026

    @api.model
    @api.onchange('agency')
    def onchange_agency(self):
        if self.agency:
            print self.agency,"Agency"
            # currency_obj = self.env['res.currency']
            payprice_obj = self.env['payment.type']
            # curr_obj = self.env['res.currency.exchange']
            ...
            if self.currency:
                for rec in self.room_lines:
                    print "Hello"
                    sale_order_ids = sale_order_line_obj.search([('id','=',rec.order_line_id.id)])
                    if sale_order_ids:
                        for order in sale_order_ids:
                            product_ids = room_obj.search([('product_id','=',order.product_id.id)])
                            partner_line_id = partner_line_obj.search([('partner_line_id','=',self.agency.id),('room_type','=',product_ids.categ_id.id),('currency','=',self.currency.id)])
                            agencys_price = partner_line_id.price
                            val={'checkin_date': rec.checkin_date,
                                'checkout_date': rec.checkout_date,
                                'product_id': rec.product_id.id,
                                'name': rec.name,
                                'price_unit': agencys_price,
                                'currency':self.currency.id,
                                'product_uom_qty':rec.product_uom_qty,}
                            rec.write(val)
                # for ser in self.service_lines:
                #     sale_order_ids = sale_order_line_obj.search([('id','=',ser.service_line_id.id)])
                #     if sale_order_ids:
                #         currency_ids = payprice_obj.search([('currency',"=",self.currency.id)])
                #         if currency_ids:
                #             self.currency_amount=currency_ids.to_amount
                #             print currency_ids.to_amount,"Currency Ammount"
                #             for order in sale_order_ids:
                #                 product_ids = product_obj.search([('id','=',order.product_id.id)])
                #                 product_ids = product_tmpl_obj.search([('id','=',product_ids.product_tmpl_id.id)])
                #                 for pro in product_ids:
                #                     order.price_unit = (pro.list_price)*(currency_ids.to_amount/currency_ids.from_amount)
                #                 if ser.currency != self.currency:
                #                     val={'ser_checkin_date': ser.ser_checkin_date,
                #                         'ser_checkout_date': ser.ser_checkout_date,
                #                         'product_id': ser.product_id.id,
                #                         'room_no': ser.room_no.id,
                #                         'name': ser.name,
                #                         'price_unit': order.price_unit,
                #                         'currency':self.currency.id,
                #                         'product_uom_qty':ser.product_uom_qty,}
                #                     ser.write(val)
    #                             else: 
    #                                 val={'ser_checkin_date': ser.ser_checkin_date,
    #                                     'ser_checkout_date': ser.ser_checkout_date,
    ...
    #                                 ser.write(val)
    #                     else:
    #                         raise Warning(_('Please Create Currency Exchange!'))

    

    # @api.onchange('currency')
    # def currncey(self):
    #     currency_obj = self.env['res.currency.exchange']
    ...
    def onchange_currency(self):
        ...
        partner_obj = self.env['res.partner']
        room_obj = self.env['hotel.room']
        service_obj =self.env['hotel.services']
        parnter_line_obj = self.env['res.partner.line']
        room_obj = self.env['hotel.room']
        if self.agency:
            print "HI"
            for rec in self.room_lines:
                ...
                if sale_order_ids:
                    for order in sale_order_ids:
                        product_ids = room_obj.search([('product_id','=',order.product_id.id)])
                        partner_line_id = parnter_line_obj.search([('partner_line_id','=',self.agency.id),('room_type','=',product_ids.categ_id.id),('currency','=',self.currency.id)])
                        agencys_price = partner_line_id.price
                        val={'checkin_date': rec.checkin_date,
                            'checkout_date': rec.checkout_date,
                            'product_id': rec.product_id.id,
            ...
            for ser in self.service_lines:
                ...
                if sale_order_ids:
                    for order in sale_order_ids:
                        product_ids = product_obj.search([('id','=',order.product_id.id)])
                        service_id = service_obj.search([('product_id','=',product_ids.id)])
                        payprice_id = payprice_obj.search([('currency','=',self.currency.id),('service_charges','=',service_id.id)])
                        val={'ser_checkin_date': ser.ser_checkin_date,
                            'ser_checkout_date': ser.ser_checkout_date,
                            'product_id': ser.product_id.id,
                            'room_no': ser.room_no.id,
                            'name': ser.name,
                            'price_unit': payprice_id.price,
                            'currency':self.currency.id,
                            'product_uom_qty':ser.product_uom_qty,}
                        ser.write(val)
        else:
            for rec in self.room_lines:
                ...
                if sale_order_ids:
                    ...
                    for order in sale_order_ids:
                        product_ids = product_obj.search([('id','=',order.product_id.id)])
                        room_ids = room_obj.search([('product_id','=',product_ids.id)])
                        payprice_id = payprice_obj.search([('currency','=',self.currency.id),('room_charges','=',room_ids.id)])
                        print "Pay",payprice_id
                        print "Pric",payprice_id.price
                        val={'checkin_date': rec.checkin_date,
                            'checkout_date': rec.checkout_date,
                            'product_id': rec.product_id.id,
                            'name': rec.name,
            ...
            for ser in self.service_lines:
                ...
                if sale_order_ids:
                    for order in sale_order_ids:
                        product_ids = product_obj.search([('id','=',order.product_id.id)])
                        service_id = service_obj.search([('product_id','=',product_ids.id)])
                        payprice_id = payprice_obj.search([('currency','=',self.currency.id),('service_charges','=',ser.service_line_id.id)])
                        print payprice_id,"id"
                        print ser.product_id.id
                        print ser.name
                        print ser.product_uom_qty
                        val={'ser_checkin_date': ser.ser_checkin_date,
                            'ser_checkout_date': ser.ser_checkout_date,
                            'product_id': ser.product_id.id,
                            'room_no': ser.room_no.id,
    ...
    def folio_room_lines(self):
        ...
        sale_order = sale_order_obj.search([('id','=',self.order_id.id)])
        order_id = sale_order
        folio_id = self.search([('order_id','=',order_id.id),])
        '''
        ...
        for room in self[0].room_lines:
            if room.product_id.id in folio_rooms:
                if folio_id.id != self.id:
                    print folio_id,self.id
...
#            To calculate additional hours in hotel room as per minutes
    ...
    def action_print(self):
        ...
        url = 'http://192.168.200.20:8080/birt/frameset?__report=hotel_folio_report.rptdesign&folio=' + str(self.id) + '&user=' + str(self.env.user.id)
        if url:
            return {
    ...
    def action_print1(self):
        ...
        # self.sent = True
        # return self.env['report'].get_action(self, 'account.report_invoice')
        url = 'http://192.168.200.20:8080/birt/frameset?__report=hotel_folio_room_report.rptdesign&folio=' + str(self.id) + '&user=' + str(self.env.user.id)
        if url:
            return {
                'type' : 'ir.actions.act_url',
                'url'  : url,
    ...
    def action_print2(self):
        ...
        # self.sent = True
        # return self.env['report'].get_action(self, 'account.report_invoice')
        url = 'http://192.168.200.20:8080/birt/frameset?__report=hotel_service_report.rptdesign&folio=' + str(self.id) + '&user=' + str(self.env.user.id)
        if url:
            return {
                'type' : 'ir.actions.act_url',
                'url'  : url,
    ...
    def action_print3(self):
        ...
        # self.sent = True
        # return self.env['report'].get_action(self, 'account.report_invoice')
        url = 'http://192.168.200.20:8080/birt/frameset?__report=sbt_hotel_service_report.rptdesign&folio=' + str(self.id) + '&user=' + str(self.env.user.id)
        if url:
            return {
                'type' : 'ir.actions.act_url',
                'url'  : url,
    ...
    @api.model
    def action_folio_create(self,vals):
        print self.id,"self"
        print vals,"ID"
        folo_line_obj = self.env['hotel.folio.line']
        sale_order_line_obj = self.env['sale.order.line']
        folio_obj = self.env['hotel.folio']
        description = None
        product_id = None
        ...
        self._cr.execute("""SELECT 
                ...
                fol.is_check as ischeck,
                fol.checkin_date as checkin_date,
                fol.checkout_date as checkout_date,
                pp.id as product_id,
            ...
            from 
                ...
                join hotel_folio fo on(so.id = fo.order_id)
                join hotel_folio_line fol on(fol.order_line_id = sol.id)
                join product_product pp on (pp.id = sol.product_id)
            where 
                fol.checkin_date is not null and
                fo.id=%s """,(vals))
        ...
        for r in res:
            ...
            # c_day = c_date.split("-")[2]
            # c_day1 =int(c_day)+1
            # print c_day,c_month,c_year
            # checkoutdate = str(datetime.datetime(int(c_year),int(c_month),c_day1))
            DATETIME_FORMAT = "%Y-%m-%d %H:%M:%S"
            date_field1 = datetime.datetime.strptime(checkout_date, DATETIME_FORMAT)
            date_field2 = date_field1+timedelta(days=1)
            checkout_date = date_field2 
            val = {'name':name,
                ...
                'state':'draft'}
            print "sale_order",val
            order_line_id = sale_order_line_obj.create(val)
            folio_id = vals[0]
            value = {'folio_id':folio_id,
                ...
                'currency':currecny}
            print "folio_line",value
            folo_line_id = folo_line_obj.create(value)
        return folo_line_id
    ...
    #     for order in self.order_id:
    #         order.state = 'sale'
    #         print order.order_line,"oooooooooooooooooooooo"
    #         order.order_line._action_procurement_create()
    #         if not order.project_id:
    #             for line in order.order_line:
    #                 if line.product_id.invoice_policy == 'cost':
...
class HotelFolioLine(models.Model):
    ...
    def _get_checkin_date(self):
        if 'checkin' in self._context:
            ...
            print time.strftime(DEFAULT_SERVER_DATETIME_FORMAT),"Check In"
    ...
    # def check_pruduct_id(self):
    #     for loop in self.order_line_id:
    #         print loop.name,"lll"
    ...
    #         print nholidays,"fnsdfds"
    #         if nholidays:
    #              raise ValidationError(_('Same room At Folio Same Days'))
    ...
    def create(self, vals, check=True):
        ...
        # room_ids = room_obj.search([('product_id','=',product)])
        # pay_ids = payment_obj.search([('room_charges','=',room_ids.id),('currency','=',currency_id)])
        # price_unit =pay_ids.price

        print self.checkin_date,"Check In"
        print vals,"VAs"
        if 'folio_id' in vals:
            # if vals['price_unit'] == 1:
 ...
 -        '''
        partner_obj = self.env['res.partner']
        partner_line_obj = self.env['res.partner.line']
        prod_cat_obj = self.env['hotel.room']
        payment_obj = self.env['payment.type']
        ...
        if self.product_id and self.folio_id.partner_id:
            type_ids = prod_cat_obj.search([('product_id','=',self.product_id.id)])
            agency_ids = partner_obj.search([('id','=',self.folio_id.agency.id),('agency','=',True)]) 
            print "Agency",agency_ids
            if not agency_ids:
                room_ids = prod_cat_obj.search([('product_id','=',self.product_id.id)])
                pay_ids = payment_obj.search([('room_charges','=',room_ids.id),('currency','=',self.folio_id.currency.id)])
                if pay_ids:
                    print pay_ids.price,"Price" 
                    self.price_unit = pay_ids.price
                    self.name = self.product_id.name
                    self.product_uom = self.product_id.uom_id
                    self.currency = self.folio_id.currency
                    tax_obj = self.env['account.tax']
                    pr = self.product_id
                    self.price_unit = tax_obj._fix_tax_included_price(pay_ids.price,
                                                                  pr.taxes_id,
                                                                  self.tax_id)
            else:
                print "A"
                print type_ids.categ_id.id,"Category"
                print self.folio_id.currency.id ,"Currency"
                print agency_ids.id,type_ids.categ_id.id,self.folio_id.currency.id
                partner_line_ids = partner_line_obj.search([('partner_line_id','=',agency_ids.id),('room_type','=',type_ids.categ_id.id),('currency','=',self.folio_id.currency.id)])
                print partner_line_ids,"Partner_line"
                ...
                tax_obj = self.env['account.tax']
                pr = self.product_id
                self.price_unit = tax_obj._fix_tax_included_price(price,
                                                                  pr.taxes_id,
                                                                  self.tax_id)
    ...
    def product_uom_change(self):
        ...
        if self.folio_id.partner_id:
            ...
            tax_obj = self.env['account.tax']
            self.price_unit = tax_obj._fix_tax_included_price(self.price_unit,
                                                              prod.taxes_id,
                                                              self.tax_id)
...
#            To calculate additional hours in hotel room as per minutes
        ...
        # rooms add from folio in all line by aungmyoswe 20181114
        hotel_room_obj = self.env['hotel.room']
        hotel_room_ids = hotel_room_obj.search([('id','=',self.folio_id.room_ids.id)])
        product_id= hotel_room_ids.product_id
        ...
        #             print self.checkin_date,"check_date",rm_line.check_in,"check_in in db",rm_line.check_out,"Check out"
...
class HotelServiceLine(models.Model):
    ...
    # Add Hotel Room in service line
    room_no = fields.Many2one('hotel.room',string="Room No",store=True)
    is_check = fields.Boolean("Check")
    currency = fields.Many2one("res.currency",string="Currency",store=True)
    ...
    def create(self, vals, check=True):
        ...
        """
        # Add unit_price from paymentprice by aungmyoswe 20181027
        # print vals
        # product = vals['product_id']
        # currency_id = vals['currency']
        # service_obj = self.env['hotel.services']
        # payment_obj = self.env['payment.type']
        # service_ids = service_obj.search([('product_id','=',product)])
        # pay_ids = payment_obj.search([('service_charges','=',service_ids.id),('currency','=',currency_id)])
        # price_unit =pay_ids.price
        if 'folio_id' in vals:
            folio = self.env['hotel.folio'].browse(vals['folio_id'])
            vals.update({'order_id': folio.order_id.id})
    ...
    def product_id_change(self):
        ...
        payprice_obj = self.env['payment.type']
        if self.product_id and self.folio_id.partner_id:
            service_id = service_obj.search([('product_id','=',self.product_id.id)])
            payprice_id = payprice_obj.search([('currency','=',self.folio_id.currency.id),('service_charges','=',service_id.id)])
            price = payprice_id.price
            self.name = self.product_id.name
            # self.price_unit = self.product_id.list_price
            self.price_unit = price
            print price,self.price_unit,"unit_price"
            self.product_uom = self.product_id.uom_id
            self.currency = self.folio_id.currency
            tax_obj = self.env['account.tax']
    ...
    def product_uom_change(self):
        ...
        if not self.product_uom:
            self.price_unit = 0.0
            return
        # for product_id_change auto change price aungmyoswe 20181028
        # self.price_unit = self.product_id.list_price
        if self.folio_id.partner_id:
            prod = self.product_id.with_context(
                lang=self.folio_id.partner_id.lang,
...
class HotelServices(models.Model):
    ...
    categ_id = fields.Many2one('hotel.service.type', string='Service Category',
                               required=True)
    product_manager = fields.Many2one('res.users', string='Product Manager')
    charges_line = fields.One2many('payment.type','service_charges',string="Service Charges",store=True,required=True)
...
class AccountInvoice(models.Model):
    ...
    def invoice_print(self):
        ...
        # return self.env['report'].get_action(self, 'account.report_invoice')
        print "self",self.id   
        url = 'http://192.168.200.20:8080/birt/frameset?__report=Shwe_Bu_Thee_Report.rptdesign&invoice=' + str(self.id)
        if not url:
           print "Hello"
...
class PaymentType(models.Model):
    ...
    room_charges = fields.Many2one('hotel.room',string = "Room Charge Type",store=True)
    service_charges = fields.Many2one('hotel.services',string = "Service Charge Type",store=True)
    ...
    # @api.model
    # @api.onchange('currency')
    # def onchange_currncey(self):
    #     if self.currency:
    #         print self.currency,"Currency"
    #         curr_obj = self.env['res.currency.exchange']
    #         currency_ids = curr_obj.search([('from_currency',"=",self.currency.id)])
    #         if currency_ids:
    #             if self.mapped('room_charges'):
    #                 for loop in self.mapped('room_charges'):
    #                     if not loop.list_price :
    #                         self.price = 0.0
    #                     else:
    #                         for loop1 in loop.charges_line:
    #                             self.price= (loop.list_price)/currency_ids.to_amount

    #             elif self.mapped('service_charges'):
    #                 print "Hello"
    #                 for loop in self.mapped('service_charges'):
    #                     if not loop.list_price :
    #                         self.price = 0.0
    #                     else:
    #                         for loop1 in loop.charges_line:
    #                             self.price= (loop.list_price)/currency_ids.to_amount
    #             elif self.mapped('laundry_charges'):
    #                 print "Hello"
    #                 for loop in self.mapped('laundry_charges'):
    #                     print loop,"l"
    #                     print currency_ids.to_amount,"Amount"
    #                     if not loop.list_price :
    #                         self.price = 0.0
    #                     else:
    #                         for loop1 in loop.charges_line:
...
class SaleOrderLine(models.Model):
    ...
    is_check = fields.Boolean('Check',default=False,store=True)
...
#     def onchange_currncey(self): 
#         payment_obj = self.env['payment.type']
#         pro_tmp_obj = self.env['product.template']
#         cm_id = self.browse(self.ids)
#         print cm_id.id,"C"
#         pay_id = payment_obj.search([('currency','=',self.currency_id.id)])
#         product_tmp_ids = pro_tmp_obj.search([('company_id','=',)])
#         print "hell0o",product_tmp_ids
#         if self.currency_id:
#             pay_id = payment_obj.search([('currency','=',self.currency_id.id)])
#             print pay_id,"p"
#             for loop in pay_id:
#                 for tmp in product_tmp_ids:
#                     tmp.list_price= loop.price 
#                     print tmp.list_price,loop.price